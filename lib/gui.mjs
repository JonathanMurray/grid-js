...
export class ImageWidget {
    ...
    draw(ctx, allowedSize, _mouse, _keydown) {
...
class TextContainer {
    ...
    _recomputeLayout() {
        ...
        while (true) {
            ...
            if (guessIdx == 0) {
                console.debug("Cannot fit text. Will put one character on each line.");
                guessIdx = 1;
            }
            while (true) {
                ...
                if (guessW > this._widthLimit && guessIdx > 1) {
                    ...
                    this._debug("Whitespace index: ", whitespaceIdx);
                    if (whitespaceIdx >= 1) {
                        guessIdx = whitespaceIdx;
                    } else {
                        this._debug(`No whitespace found to break out a new line: '${remaining}'`);
                    }
                    assert(guessIdx > 0);
...
class Button {
    ...
    async draw(ctx, available, mouse, keydown) {
...
class SelectionList {
    constructor(onSelection, {expandHor = Expand.NO, borderColor = null, verticalScroll = false, maxHeight = Number.MAX_SAFE_INTEGER} = {}) {
        ...
        this._itemCount = 0;
        this._onSelection = (itemIdx) => {
            this._selectedIdx = itemIdx;
            if (this._selectedContainer != null) {
                this._selectedContainer._bgColor = null;
                this._selectedContainer._hoverBgColor = "#666";
            }
            this._selectedContainer = this._container._children[itemIdx];
            this._selectedContainer._bgColor = "#999";
            this._selectedContainer._hoverBgColor = "#999";
            onSelection(itemIdx);
        };
    ...
    changeSelectedIndexByDelta(delta) {
        if (this._selectedIdx == null) {
            this._selectedIdx = 0;
        } else {
            this._selectedIdx = this._modulo(this._selectedIdx + delta, this._itemCount);
        }
        this._onSelection(this._selectedIdx);
    }
...
class Container {
    ...
    async draw(ctx, available, mouse, keydown, overrides={}) {
        ...
        for (let child of this._children) {
            ...
            if (isChildVisible) {
                [childW, childH] = await child.draw(ctx, allowedChildSize, convertMousePosition(mouse, [childX, childY]), keydown, overrides);
            }
...
export async function* runEventLoop(rootElement, socketFd, canvas) {
    init(rootElement, socketFd, canvas);

    while (true) {
        for await (const event of getEvents()) {
            yield event;

            if (event.name == "closeWasClicked") {
                return;
            }
        }
    }
...
export async function init(rootElement, socketFd, canvas) {
...
export async function* getEvents(timeoutMillis) {

    if (timeoutMillis != null) {
        const readyFd = await syscall("pollRead", {fds: [STATE.socketFd], timeoutMillis});
        if (readyFd != STATE.socketFd) {
            return;
        }
    }

    const received = await read(STATE.socketFd);
    const messages = JSON.parse(received);
    for (const {name, event} of messages) {
        if (name == "mousemove") {
            STATE.mousePos = [event.x, event.y];
        } else if (name == "mousedown") {
            await redraw({changedToDown: true});    
        } else if (name == "mouseup") {
            await redraw({changedToUp: true});
        } else if (name == "wheel") {
            await redraw({scrollDelta: event.deltaY});
...
async function redraw(mouseEvent={}, keydownEvent={}) {
    STATE.ctx.resetTransform();
    //STATE.root.computeLayout();
    await STATE.root.draw(STATE.ctx, [STATE.canvas.width, STATE.canvas.height], {...mouseEvent, pos: STATE.mousePos}, keydownEvent);
}
