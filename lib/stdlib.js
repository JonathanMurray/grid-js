const stdlib = function() {
    ...
    async function createWindow(title, size, options, args) {
        ...
        if (options != undefined) {
            ...
            if ("menubarButtons" in options) {
                menubarButtons = options.menubarButtons;
            }
        }

        const canvas = await syscall("graphics", {title, size, resizable, menubarButtons});
    ...
    const terminal = function() {
        ...
        async function getCursorPosition() {
            ...
            assert(responseMatch, "Invalid cursor position response: " + response);
