...
const gui = function () {
    ...
    class TextContainer {
        constructor(ctx, text, {color = "white"} = {}) {
            this._ctx = ctx;
            this._text = text;
            this._color = color;
        ...
        draw(ctx, allowedSize, mouse) {
            ...
            if (mouse && mouse.pos && contains([0, 0, w, h], mouse.pos)) {
                ...
                if (this._onClick != null && mouse.changedToDown) {
                    EVENT_QUEUE.push(this._onClick);
                }
            }
    ...
    class Container {
        ...
        demandedSize() {
            assert (this._childrenMinSize != null);
        ...
        draw(ctx, allowedSize, mouse, overrides={}) {
            ...
            let childrenOffset = [0, this._yScrollOffset];
            ...
            if (this._childrenMinSize[1] <= h) {
                ...
                //console.log("got no spare h", this); //TODO
                clipRect = [0, 0, w, h];
                
                if (this._verticalScroll && mouse && mouse.scrollDelta != undefined) {
                    //console.log("mouse scroll: ", mouse.scrollDelta);
                    const delta = Math.sign(mouse.scrollDelta) * 30;
                    this._yScrollOffset = Math.max(
                        Math.min(this._yScrollOffset - delta, 0),
